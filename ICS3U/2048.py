...
def drawBoard():
    ...
    for i in range(4):
        for j in range(4):
            Rect(10 + 100*i, 10 + 100*j, 80, 80, fill=rgb(205, 195, 180))
    pass
...
def reverse(cellBlock):
    ...
    for i in range(4):
        reversedCellBlock.append(cellBlock.pop())
...
def moveHorizontal(direction):
    ...
    for rowIndex in range(len(app.board)):
        if (direction == 'right'):
            ...
            # Reverse the row then slide left using move. Then reverse the resulting row.
            row = app.board[rowIndex]
            newRow = reverse(move(reverse(row)))
...
def moveCols(board):
    for colIndex in range(len(board[0])):
        ...
        curr = getCol(board, colIndex)
        # Then copy the values to the board.
        curr[colIndex] = move(curr[colIndex])
        
        for i in range(4):
            board[i][colIndex] = curr[colIndex][i]
